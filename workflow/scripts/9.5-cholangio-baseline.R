...
# model
design <- model.matrix(~ labels + patientid) # 1280 DMRs with FDR<0.05
...
hyper <- res[res$logFC > 0 & res$adj.P.Val < 0.1,] #262
hypo <- res[res$logFC < 0 & res$adj.P.Val < 0.1,] #1018
...
# plot gene features
filename <- paste0("data/results/figures/cholangio/FINAL_all_sdpd_peakanno.png")
